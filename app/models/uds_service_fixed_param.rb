class UdsServiceFixedParam < ActiveRecord::Base
  ...
  def to_serv_fixparams_c(index,is_bootloader=false)
    ...
    retswitch +="        case "+prefix+self.complete_c_define_name+"_ID: 
            if (UDSServWithFixParams_check_permissions("+prefix+self.complete_c_define_name+"_NUMBYTE,"+prefix+self.complete_c_define_name+"_BITMASK ,&response_mode)==TRUE){ 
                if (UDSServWithFixParams_"+self.complete_c_name+"(data_buffer,resp_pos,&data_size)==TRUE){ 
  ...
  def complete_c_name
    if uds_sub_service then
      ret=""+uds_sub_service.c_name+""+c_name
    else
      if uds_service the
        ret=""+uds_service.c_name+""+c_name
      else
        ret=c_name
      end
    end
  ...
  def to_serv_fixparams_h(prev_serv_fixparams,is_bootloader=false)
    ...
    ret+="#define "+prefix+self.complete_c_define_name+"_ID                 ((uint16_t)0x"+self.ident.upcase+"U)\n"
    ...
    ret+="#define "+prefix+self.complete_c_define_name+"_DATA_SIZE           ((uint16_t)"+self.data_size.to_s+")\n"
    ...
    ret+="#define "+prefix+self.complete_c_define_name+"_SESSION_ACCESS     (((uint8_t)0x00)"
    if ((!is_bootloader and self.app_session_default) or (is_bootloader and self.boot_session_default)) then
    ...
    ret+="#define "+prefix+self.complete_c_define_name+"_SECURITY_ACCESS     (((uint8_t)0x00)"
    if (self.sec_locked) then
    ...
    ret+="#define "+prefix+self.complete_c_define_name+"_ADDRESSING_ACCESS     (((uint8_t)0x00)"
    if (self.addr_phys) then
  ...
  def to_routine_ctrl_h(prev_serv_fixparams,is_bootloader=false)
    if (!is_bootloader) then
      prefix="UDS_SERV_FIXPARAMS_"
    else
      prefix="UDS_BL_SERV_FIXPARAMS_"
...
retfunc+="/**\n"
retfunc+=" * RoutineCtrl "+self.name+"\n"
retfunc+=" * @param routine_entry_option entry option\n"
retfunc+=" * @param size size of the request\n"
retfunc+=" * @param resp routine response\n"
retfunc+=" * @return response mode\n"
retfunc+=" */\n"   
    retfunc+="uint8_t UdsRoutineCtrl"+self.complete_c_name+"(uint8_t *routine_entry_option, uint16_t size, tp_uds_control_routine resp);\n"
    if (self.configuration_switch!=nil) then
      retfunc+="#endif\n"
    end
  ...
  def to_routine_ctrl_c(index,is_bootloader=false)
    ...
    ret+="uint8_t UdsRoutineCtrl"+self.complete_c_name+"(uint8_t *routine_entry_option, uint16_t size, tp_uds_control_routine resp)\n{\n"
    ret+="\tuint8_t response_mode=/* Set positive response */;\n"
    ...
    retswitch += "\telse if (id=="+prefix+self.complete_c_define_name+"_ID)\n\t{
                if (routine_ctrl_type == 0x"+self.uds_sub_service.ident+")
                {
                  if (/* Suppress positive response */ == FALSE) {
                    if (UdsCheckPermissionsRoutine("+prefix+self.complete_c_define_name+"_SESSION_ACCESS,"+prefix+self.complete_c_define_name+"_SECURITY_ACCESS ,"+prefix+self.complete_c_define_name+"_ADDRESSING_ACCESS ,&response_mode)==TRUE){ 
                      if (size == "+prefix+self.complete_c_define_name+"_DATA_SIZE){
                        response_mode = UdsRoutineCtrl"+self.complete_c_name+"(routine_entry_option, size, resp);
                      } else {
