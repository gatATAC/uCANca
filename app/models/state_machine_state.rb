class StateMachineState < ActiveRecord::Base
  ...
  has_many :sub_machines, -> { limit(1) }, :class_name => 'StateMachine', :inverse_of => :super_state, :foreign_key => :super_state_id
  has_many :state_machine_transitions, -> { order "priority DESC"}, :inverse_of => :state_machine_state, :dependent => :destroy
